...
class Window extends EventTarget {
    ...
    constructor(title, url, width, height) {
        ...
        if (vm.isContext(this)) {
            console.log('already context')
        }
        vm.createContext(this);
        vm.runInContext('this', this)
        vm.runInContext("globalThis.window = this", this);
        ...
        this.location.ownerWindow = this;
    ...
    alert(message = '') { console.log('alert'); }
    confirm(message = '') { console.log('confirm'); }
    prompt(message = '') { console.log('prompt'); }

    close() { global.app.closeWindow(this) }

    atob(encodedStr) {
        let buff = Buffer.from(encodedStr);
