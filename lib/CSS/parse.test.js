...
describe('parse', function() {
	...
	TESTS.forEach(function(tc) {
		it(tc.input, function() {
			const input = tc.input;
			var parsed = parseStyleSheet(input);

			// Performance could optimized in order of magnitude but it’s alreaddy good enough
			uncircularOwnProperties(parsed);
			uncircularOwnProperties(tc.result);
			removeUnderscored(parsed);
			removeUnderscored(tc.result);

			expect(objectsHaveSameOwnProperties(parsed, tc.result)).toBe(true)
		});
	});

	it('a{content:"\\""}', function() {
		const input = 'a{content:"\\""}'
		var parsed = parseStyleSheet(input);
	...
	it('a{content:"abc\\"\\"d\\"ef"}', function() {
		const input = 'a{content:"abc\\"\\"d\\"ef"}'
		var parsed = parseStyleSheet(input);
